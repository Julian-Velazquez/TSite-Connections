...
//this endpoint is used for search requests for records with a specific manufacturer type
...
//data validation using regex and checking length. ensures only numeric characters for the manufacturer id and also that the id is of a specific length
...
if($stat==0){ // shows active (default)
	
	//this sql query essentially returns rows where the record's manufacturer id matches the given manufacturer id. however it only allows records that are active
	...
	WHERE manufacturer_id = '$mid' AND inactive.sn_id IS NULL
	LIMIT 1000";
	
}else if($stat==1){ //shows all
	
	//this sql query essentially returns rows where the record's manufacturer id matches the given manufacturer id. this one allows records that are active AND inactive
...
}else if($stat==2){ //shows inactive
	
	//this sql query essentially returns rows where the record's manufacturer id matches the given manufacturer id. however it only allows records that are inactive
...
$datarr=array();
if ($rst->num_rows>0){//rows/records with the given manufacturer id are found 
