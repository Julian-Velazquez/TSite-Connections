<?php
//endpoint that handles sending out a list of all manufacturers (both active and inactive)
try{
...
$sql="Select `name`, `auto_id` from `manufacturer`";
//tries to query the database for list of all manufacturers, if it fails it sends out the written error message
try{
$result=$dblink->query($sql);
    ...
    $output[]='Action: listALL_manufacturer';
...
die();
    ...
    $output[]='Action: listALL_manufacturers';
    $responseData=json_encode($output);
    echo $responseData;
    die();
}
